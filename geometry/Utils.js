...
export const rotatePoint = (point, pivot, angle) => {
  ...
  return vec2.set(
    vec2.create(),
    Math.floor(xnew + pivot.x),
    Math.floor(ynew + pivot.y)
  );
};
...
export const polygonCollision = (polygonA, polygonB) => {
  ...
  for (let index = 0; index < edgeCountA + edgeCountB; index += 1) {
    const edge =
      index < edgeCountA
        ? polygonA.edges[index]
        : polygonB.edges[index - edgeCountA];

    // Find the axis perpendicular to the current edge
    const axis = vec2.normalize(
      vec2.create(),
      vec2.set(vec2.create(), -edge.y, edge.x)
    );
